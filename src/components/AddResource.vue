  ...
  <Teleport to="body">
    <v-snackbar v-model="showSnack" color="white">
      ...
      <template v-slot:actions>
        <v-btn color="red" variant="text" @click="showSnack = false">
          Close
        </v-btn>
      </template>
    </v-snackbar>
    ...
    <template #addResource>
      <v-card-text>
        <form @submit.prevent="submitForm">
          ...
          <v-btn flat class="bg-purple-900 text-white" rounded="0" type="submit"
            >Submit</v-btn
          >
        </form>
      </v-card-text>
...
export default {
  ...
  computed: {
    getColor() {
      return this.message == "Failed to submit form!"
        ? "text-red"
        : "text-green";
    },
  },
